...
void BlurApp::Draw(const GameTimer& gt)
{
	...
	mSobelFilter->Execute(mCommandList.Get(), mPostProcessRootSignature.Get(),
		mPSOs["sobel"].Get(), CurrentBackBuffer());
	...
	mCommandList->SetGraphicsRootDescriptorTable(2, mSobelFilter->OutputSrv());
...
void BlurApp::BuildDescriptorHeaps()
{
	...
	const int sobelDescriptorCount = 3;

	//
	// Create the SRV heap.
	//
	D3D12_DESCRIPTOR_HEAP_DESC srvHeapDesc = {};
	srvHeapDesc.NumDescriptors = textureDescriptorCount + blurDescriptorCount + sobelDescriptorCount;
	...
	mBlurFilter->BuildDescriptors(
		CD3DX12_CPU_DESCRIPTOR_HANDLE(srvCpuStart, textureDescriptorCount, mCbvSrvUavDescriptorSize),
		CD3DX12_GPU_DESCRIPTOR_HANDLE(srvGpuStart, textureDescriptorCount, mCbvSrvUavDescriptorSize),
		mCbvSrvUavDescriptorSize);

	mSobelFilter->BuildDescriptors(
		CD3DX12_CPU_DESCRIPTOR_HANDLE(srvCpuStart, textureDescriptorCount + blurDescriptorCount, mCbvSrvUavDescriptorSize),
		CD3DX12_GPU_DESCRIPTOR_HANDLE(srvGpuStart, textureDescriptorCount + blurDescriptorCount, mCbvSrvUavDescriptorSize),
		mCbvSrvUavDescriptorSize);
}
