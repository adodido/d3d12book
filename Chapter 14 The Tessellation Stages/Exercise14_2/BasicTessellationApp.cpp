...
void BasicTessellationApp::BuildSphereGeometry()
{
	GeometryGenerator geoGen;
	GeometryGenerator::MeshData sphere = geoGen.CreateGeosphere(1.f,0);
	...
	sphereSubmesh.IndexCount = indices.size();
	sphereSubmesh.StartIndexLocation = 0;
	sphereSubmesh.BaseVertexLocation = 0;

	geo->DrawArgs["spherepatch"] = sphereSubmesh;
...
void BasicTessellationApp::BuildRenderItems()
{
	...
	auto tessSphereRitem = std::make_unique<RenderItem>();
	tessSphereRitem->World = MathHelper::Identity4x4();
	tessSphereRitem->TexTransform = MathHelper::Identity4x4();
	tessSphereRitem->ObjCBIndex = 0;
	tessSphereRitem->Mat = mMaterials["whiteMat"].get();
	tessSphereRitem->Geo = mGeometries["tessSphereGeo"].get();
	tessSphereRitem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_3_CONTROL_POINT_PATCHLIST;
	tessSphereRitem->IndexCount = tessSphereRitem->Geo->DrawArgs["spherepatch"].IndexCount;
	tessSphereRitem->StartIndexLocation = tessSphereRitem->Geo->DrawArgs["spherepatch"].StartIndexLocation;
	tessSphereRitem->BaseVertexLocation = tessSphereRitem->Geo->DrawArgs["spherepatch"].BaseVertexLocation;
	mRitemLayer[(int)RenderLayer::Opaque].push_back(tessSphereRitem.get());

	mAllRitems.push_back(std::move(tessSphereRitem));
}
